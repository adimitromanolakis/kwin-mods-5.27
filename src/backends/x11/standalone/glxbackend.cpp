...
uint64_t c1 = timestamp.count();

double diff = (c1 - last_c1) / 1000.0;

//if(abs(diff - 16.6) > 2)
//qWarning() << "SWAP: diff:" << diff;
...
void GlxBackend::init()
{
    ...
    fprintf(stderr,"APO glPreferBufferSwap final is %c\n", options->glPreferBufferSwap());
    ...
    printf("XXX APO14 supportsSwapEvent = %d\n", supportsSwapEvent);
    ...
    static int useVsyncProvider = qEnvironmentVariableIntValue("KWIN_X11_VSYNC_PROVIDER");
    qWarning() << "SWAP: KWIN_X11_VSYNC_PROVIDER = " << useVsyncProvider;

    static bool forceSoftwareVsync = qEnvironmentVariableIntValue("KWIN_X11_FORCE_SOFTWARE_VSYNC");
    if (supportsSwapEvent && !forceSoftwareVsync && useVsyncProvider == 2) {
        ...
        qWarning() << "SWAP: using SwapEventFilter";
        ...
        if (!forceSoftwareVsync) {
            if(useVsyncProvider == 1) if (!m_vsyncMonitor) {
                m_vsyncMonitor = SGIVideoSyncVsyncMonitor::create(this);
                qWarning() << "SWAP: using SGIVideoSyncVsyncMonitor";
            }
            if(useVsyncProvider == 0) if (!m_vsyncMonitor) {
                m_vsyncMonitor = OMLSyncControlVsyncMonitor::create(this);
                qWarning() << "SWAP: using OMLSyncControlVsyncMonitor";
    ...
    qCWarning(KWIN_X11STANDALONE) << "Direct rendering:" << isDirectRendering();
}
...
printf("ZZZZZZ: fbconfig\n");
    qCCritical(KWIN_X11STANDALONE, "FBCONFIG: Choosing GLXFBConfig %#x X visual %#x depth %d RGBA %d:%d:%d:%d ZS %d:%d sRGB: %d",
...
FBConfigInfo *GlxBackend::infoForVisual(xcb_visualid_t visual)
{
    ...
    printf("ZZZZZZ: choose fbconfig n=%d\n", count);
    qCCritical(KWIN_X11STANDALONE, "FBCONFIG: Choosing GLXFBConfig  dred=%d\n", red_bits);
...
void GlxBackend::setSwapInterval(int interval)
{
    ...
    char *env1 = getenv("GLX_SWAP");
    
    if(env1) { 
        interval = atoi(env1);
    }

    printf("XXX APO11 GlxBackend::setSwapInterval(int interval) %d\n",interval);
