...
double interp_power = 5;

static double interpolate2(double x, double y, double a)
{
    double exp_mult = 1;

    //if(interp_power >= 1) a = 1-a;

    if(0) {
        exp_mult= interp_power;

        a = exp(exp_mult*a);

        double a0 = exp(0);
        double a1 = exp(exp_mult*1);

        a = (a-a0)/(a1-a0);

    } else {

        a = pow(a,interp_power);
    }
...
void GlideEffect::paintWindow(EffectWindow *w, int mask, QRegion region, WindowPaintData &data)
{
    ...
    //{ FILE *f = fopen("/tmp/i.txt","r"); double b; fscanf(f, "%lf", &b); fclose(f); interp_power = b;
    //}
    interp_power = 0.5;

    //fprintf(stderr,"INTERP POWER=%f\n", interp_power);
    ...
    case RotationEdge::Top:
        ...
        data.setRotationAngle(-interpolate2(params.angle.from, params.angle.to, t));
        break;
    ...
    data.setZTranslation(-interpolate2(params.distance.from, params.distance.to, t));
    data.multiplyOpacity(interpolate2(params.opacity.from, params.opacity.to, t));
