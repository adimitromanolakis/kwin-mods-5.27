...
bool GLShader::link()
{
    ...
    if (status == 0) {
        ...
        qCWarning(LIBKWINGLUTILS) << "Shader link log:" << log;
    }
...
bool GLShader::compile(GLuint program, GLenum shaderType, const QByteArray &source) const
{
    ...
    //QString s = QString::fromLatin1(log);
    //if(status == 0) GLPlatform::instance()->globalShaderCompileLog = log;
    if(status == 0) fprintf(stderr,"Shader log=%s\n", log.constData());
    ...
    if (status == 0) {
        ...
        qCWarning(LIBKWINGLUTILS) << "Shader compile log:" << log;
...
void GLShader::resolveLocations()
{
    ...
    mFloatLocation[Random]    = uniformLocation("g_Random");
