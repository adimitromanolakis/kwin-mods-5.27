...
void Options::setGlPreferBufferSwap(char glPreferBufferSwap)
{
    ...
    qWarning() << "XXX DBEUG: setGlPreferBufferSwap  = " << (glPreferBufferSwap);
    glPreferBufferSwap = CopyFrontBuffer;

    qWarning() << "XXX4 setGlPreferBufferSwap glbufferswap = "  << 
        ((glPreferBufferSwap == CopyFrontBuffer)?"CopyFrombuffer":"") << 
        ((glPreferBufferSwap == ExtendDamage)?"ExtendDamage":"") << 
        "Driver=" << (int) GLPlatform::instance()->driver() << 
        " ChipClass = " << (int) GLPlatform::instance()->chipClass();
...
void Options::reloadCompositingSettings(bool force)
{
    ...
    printf("Set prefer buffer swap to %c\n", c);
